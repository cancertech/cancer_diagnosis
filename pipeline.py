...
def process_img(infname, output_base_dir, num_pixels_per_seg, slic_shrink_scale, 
    ...
    he = utils.color_conversion.rgb2he(img).astype(np.uint8)
    imageio.imsave(os.path.join(output_dir, 'he.jpg'), he)
    img_features = utils.feature_extraction.extract_features_for_slice(img)
    ...
    utils.viz.viz_segmentation_countour(img, img_label, border_color=[1, 0, 0], output_dir=output_dir)
    plt.imshow(img_label)

            
    X = utils.feature_extraction.hist_features_for_all_superpixel(img_features, img_label)
    ...
    utils.viz.save_superpixels(img, img_label, kmeans.labels_, output_dir=output_dir)
    
    
    utils.viz.superpixel_in_html(output_dir=output_dir)
